...
// Mirrors IncidentAttachmentService's validation/storage approach. Unlike
// incident attachments, there's no parent id to check at upload time — the
// CPD activity doesn't exist yet when the form's file picker uploads —
// so this is a standalone upload: it just validates, stores, and returns an
// id, which the form then sends back as CpdActivityCreateDto.AttachmentId.
public class CpdActivityAttachmentService : ICpdActivityAttachmentService
{
    ...
    public CpdActivityAttachmentService(AppDbContext db, ICurrentUserService currentUser, IConfiguration configuration, IWebHostEnvironment env)
    {
        _db = db;
    ...
    public async Task<AttachmentUploadDto> UploadAsync(IFormFile file, CancellationToken cancellationToken)
    {
        ...
        var relativeFolder = Path.Combine("cpd-activities", "attachments");
        ...
        var attachment = new CpdActivityAttachment
        {
        ...
        _db.CpdActivityAttachments.Add(attachment);
    ...
    {
        var attachment = await _db.CpdActivityAttachments
